...
require_once('/var/www/common/socket.php');
	...
	*/
	...
	function Connect ($server = '') {
		...
		while($retry <= $this->RETRY) {
			...
			if(isset($this->Query['src_ip']))
                $src_ip=$this->Query['src_ip'];
            elseif(isset($this->src_ip))
                $src_ip=$this->src_ip;
            else
                $src_ip=false;
			$ptr=connect($server, $port, $src_ip);
			if($ptr > 0) {
	...
	function Process (&$result, $deep_whois=true) {
        ...
        if(isset($this->Query['src_ip']))
            $handler->src_ip = $this->Query['src_ip'];
